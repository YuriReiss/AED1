...
int main () {
	...
	while(1){
		...
		scanf("%s%*c", str);
		if(strcmp(str,"fim")){
			if(!strcmp(str,"(") || !strcmp(str,"{") || !strcmp(str,"[")) pilha_insere(p,str);
			else if(!strcmp(str,")")){
				if(strcmp(pilha_obtem_topo (p),"(")) teste=1;
				else pilha_remove(p);
			}else if(!strcmp(str,"}")){
				if(strcmp(pilha_obtem_topo (p),"{")) teste=1;
				else pilha_remove(p);
	...
	if(pilha_se_vazia(p)==1 && teste==0) printf("sim\n");
	else if(pilha_se_vazia(p)!=1 || teste==1)printf("nao\n");			
	pilha_libera(p);
