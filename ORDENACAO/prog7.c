	...
	for(l=0;l<t;l++){
		...
		for(i=0;i<n-1;i++){
			if(horario[i][0]==horario[i+1][0]){
				if((horario[i+1][1]-minutosReferenciais)>=1){//If utilizado para manipular o contador					
					cont=1;					
					minutosReferenciais=horario[i+1][1];	
					iTeste=i;				
					while((minutosReferenciais-horario[i][1])<1) i--;			
					if(iTeste!=i) i++;
				}
				if((horario[i+1][1]-horario[i][1])<1) cont++;
